...
function sendEvent(client) {
  ...
  const payload = {
    ...
    client: EventNames.client2,
  };
  console.log(
    `Client 2 - ${event.type} - ${event.date} - ${event.account} - ${event.amount} , ${event.transactionId}`
  );
  client.emit(EventNames.send, payload);
}

function startClient(client) {
  console.log('Client 2 started ...');
  client.emit(EventNames.receive, (payload) =>
    console.log(`Client 2 - ${payload} has been received`)
  );
